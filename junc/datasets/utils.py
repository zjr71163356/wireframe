...
def ann_to_array(H, d, max_len=1):
    ...
    junction_residual = np.zeros((grid_h, grid_w, max_len, 2), dtype=np.float64)
    ...
    theta_bin_residual = np.zeros(
        (grid_h, grid_w, max_len, H['num_bin']), dtype=np.float64)
...
def load_data_and_save(H, src_dir, save_dir, filenames, split='train', use_mp=True):
    ...
    def loadDataset():
        ...
        return Parallel(n_jobs=1)(
            delayed(loading)(f, src_dir) for f in tqdm(filenames, desc="Loading data")
        )
    ...
    if not debug:
        ...
        if use_mp:
            ...
            Parallel(n_jobs=cpu_num)(
                ...
                for d in tqdm(anno, desc="Saving annotations")
            )
        else:
